...
export class DragStack {
  ...
  _onMouseUp(e) {
    ...
    if (target) {
      ...
      this.onDrop(target.dataset.tagId, this.selection.ids);
    } else {
      this._animateCancel();
    }
  ...
  _startDrag(e) {
    ...
    previewCards.forEach((card, i) => {
      ...
      if (i > 0) {
        ...
        ], { duration: 300 + i * 50, delay: i * 40, easing: SPRING_EASING, fill: 'forwards' });
      }
    });

    const badge = document.createElement('div');
    badge.className = 'stack-badge';
    badge.textContent = `${selectedIds.length} Gots`;
    badge.style.cssText = `
    ...
    this._bobAnim = floatBob(this._stackEl);
    this._moveStack(e.clientX, e.clientY);
    ...
    ], { duration: 280, easing: SPRING_EASING });
  }
  ...
  _animateDrop(x, y, targetEl) {
    ...
    minis.forEach((mini, i) => {
      ...
      burst(mini, angle, 60 + Math.random() * 40);
    });

    if (targetEl) tagReceive(targetEl);
  ...
  _animateCancel() {
    ...
    shake(this._stackEl).finished.then(() => {
      this._destroyStack();
      document.body.classList.remove('is-dragging');
      const dropZone = document.getElementById('tag-drop-zone');
      if (dropZone) dropZone.style.display = 'none';
