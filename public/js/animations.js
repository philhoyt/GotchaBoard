...
// Shared Web Animations API helpers

const SPRING_EASING = 'cubic-bezier(0.34, 1.56, 0.64, 1)';
const EASE_OUT      = 'cubic-bezier(0.25, 0.46, 0.45, 0.94)';

function springIn(el, fromScale = 0.8, duration = 280) {
  return el.animate([
...
function springOut(el, toScale = 0.8, duration = 200) {
  return el.animate([
    { transform: 'scale(1)',            opacity: 1   },
...
// Fly an element from its current position to (targetX, targetY) with rubber-band overshoot
function flyTo(el, targetX, targetY, rotation = 0, duration = 350) {
...
// Burst: element flies outward then shrinks away
function burst(el, angle, distance = 80, duration = 450) {
...
// Shake an element left/right (no-drop indication)
function shake(el, duration = 480) {
  return el.animate([
...
// Tag receive — bounce + flash
function tagReceive(el, duration = 400) {
...
// Float bob (infinite loop for drag stack)
function floatBob(el) {
  return el.animate([
...
// Sweep card off-screen (bulk action complete)
function sweep(el, delay = 0, duration = 350) {
  return el.animate([
...
window.Animations = { springIn, springOut, flyTo, burst, shake, tagReceive, floatBob, sweep, SPRING_EASING };
